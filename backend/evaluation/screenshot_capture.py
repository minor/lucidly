...
class ScreenshotCapture:
    ...
    async def capture_url_iframe(
        ...
        print(f"Capturing iframe from {page_url}")
        try:
            # load the page first (critical)
            ...
            # if you want container screenshot instead
            ...
            # otherwise capture iframe
            iframe = page.locator(iframe_selector).first

            await iframe.wait_for(state="visible", timeout=60000)
            await iframe.scroll_into_view_if_needed()

            # allow external embed to render
            await asyncio.sleep(wait_time)
